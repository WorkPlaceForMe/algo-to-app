...
class FileVideoStream_video:
    def __init__(self, path, screenXY, restart, wait):
        self.stream = cv2.VideoCapture(path)
        self.path = path
...
def createFileVideoStream(mode, path, screenXY, restart, wait=.05):
    if mode == 'auto' and (path[:4] == 'rtsp' or path[:4] == 'http'):
        return FileVideoStream_live(path, screenXY)
    elif mode == 'live':
        return FileVideoStream_live(path, screenXY)
    else:
        return FileVideoStream_video(path, screenXY, restart, wait)
